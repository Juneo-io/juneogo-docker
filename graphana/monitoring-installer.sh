#!/bin/bash
# Does a multi-step installation of Prometheus, Grafana, node_exporter and Juneo dashboards
...
usage () {
  ...
  echo "   --4      Step 4: Installs Juneogo Grafana dashboards"
  echo "   --5      Step 5: (Optional) Installs additional dashboards"
  echo ""
  echo "Run without any options, script will download and install latest version of Juneogo dashboards."
}
...
install_prometheus() {
  echo "Juneogo monitoring installer"
  echo "--------------------------------"
  echo "STEP 1: Installing Prometheus"
  echo
  get_environment
  check_reqs
  mkdir -p /tmp/juneo-monitoring-installer/prometheus
  cd /tmp/juneo-monitoring-installer/prometheus
...
install_exporter() {
  ...
  echo
  get_environment
  mkdir -p /tmp/juneo-monitoring-installer/exporter_archive
  cd /tmp/juneo-monitoring-installer/exporter_archive
  ...
  tar xvf node_exporter.tar.gz -C /tmp/juneo-monitoring-installer/exporter_archive --strip-components=1
  sudo mv /tmp/juneo-monitoring-installer/exporter_archive/node_exporter /usr/local/bin
...
install_dashboards() {
  ...
  if test -f "/etc/grafana/provisioning/dashboards/juneo.yaml"; then
  ...
  echo
  echo "Downloading..."
  mkdir -p /tmp/juneo-monitoring-installer/dashboards-install
  cd /tmp/juneo-monitoring-installer/dashboards-install
  ...
  if [ "$provisioningDone" = "false" ]; then
    ...
    sudo cp juneo.yaml /etc/grafana/provisioning/dashboards/
  ...
  echo "Done!"
  echo
  echo "Juneogo Grafana dashboards have been installed and updated."
